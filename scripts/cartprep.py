...
def shift_all(
    tri_files, x_shift, y_shift, z_shift, component: str = None, reverse: bool = False
):
    ...
    for file in transform_files:
        ...
        if reverse:
            os.system(
                f"trix -x {-x_shift} -y {-y_shift} -z {-z_shift} -o {prefix} {file}"
            )
        else:
...
def rotate_all(
    tri_files, x_rot, y_rot, z_rot, component: str = None, reverse: bool = False
):
    if component is None:
        transform_files = tri_files
    else:
...
if __name__ == "__main__":
    ...
    if intersected:
        ...
        with open("temp", "w+") as new_file:
            ...
            shutil.move(os.path.join("temp"), os.path.join("input.cntl"))
