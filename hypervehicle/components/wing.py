    ...
    if len(wing_geometries) > 0:
        ...
        for wing_no, wing_geometry in enumerate(wing_geometries):
            ...
            if wing_geometry["Line_B0TT_TYPE"].lower() == "bezier":
                ...
                raise Exception(
                    "Option for 'Line_B0TT'={} not supported.".format(
                        wing_geometry["Line_B0TT_TYPE"]
                    )
                )
            ...
            for i in range(2):
                top_patch[i] = OffsetPatchFunction(
                    wing_patch_flipped[i], function=wing_geometry["FUNC_TOP_THICKNESS"]
                )
                bot_patch[i] = OffsetPatchFunction(
                    wing_patch[i], function=wing_geometry["FUNC_BOT_THICKNESS"]
                )
            ...
            top_edge_path = OffsetPathFunction(
                Line_B0TT, wing_geometry["FUNC_TOP_THICKNESS"]
            )
            bot_edge_path = OffsetPathFunction(
                Line_B0TT, wing_geometry["FUNC_BOT_THICKNESS"]
            )
            ...
            if "LE_TYPE" in wing_geometry and wing_geometry["LE_TYPE"] == "FLAT":
                patch_dict[f"w{wing_no}_LE_patch0"] = FlatLeadingEdgePatchFunction(
                    top_edge_path, bot_edge_path, 0, t_B1
                )
                patch_dict[f"w{wing_no}_LE_patch1"] = FlatLeadingEdgePatchFunction(
                    top_edge_path, bot_edge_path, t_B1, t_B2
                )
                patch_dict[f"w{wing_no}_LE_patch2"] = FlatLeadingEdgePatchFunction(
                    top_edge_path, bot_edge_path, t_B1, 1
                )
            ...
            if wing_geometry["TAIL_OPTION"] == "FLAP":
                if verbosity > 1:
                    print(
                        "        Flap length = {}".format(wing_geometry["FLAP_LENGTH"])
                    )
                    print(
                        "        Flap angle  = {}".format(wing_geometry["FLAP_ANGLE"])
                    )

                # Define top and bottom TE paths
                TE_top = TrailingEdgePath(
                    A0, B0, thickness_function=wing_geometry["FUNC_TOP_THICKNESS"]
                )
                TE_bot = TrailingEdgePath(
                    A0, B0, thickness_function=wing_geometry["FUNC_BOT_THICKNESS"]
                )
                ...
                if "LE_TYPE" in wing_geometry and wing_geometry["LE_TYPE"] == "FLAT":
                    ...
                    elipse_top = ElipsePath(
                        centre=TE_mean_line(1),
                        thickness=thickness_top,
                        LE_width=wing_geometry["FUNC_LEADING_EDGE_WIDTH"](0.0),
                        side="top",
                    )
                ...
                south = Line(
                    p0=Vector3(x=TE_mean_line(1).x, y=TE_mean_line(1).y, z=TE_top(1).z),
                    p1=Vector3(
                        x=TE_mean_line(1).x
                        - wing_geometry["FLAP_LENGTH"]
                        * np.cos(wing_geometry["FLAP_ANGLE"]),
                        y=TE_mean_line(1).y,
                        z=TE_mean_line(1).z
                        + wing_geometry["FLAP_LENGTH"]
                        * np.sin(wing_geometry["FLAP_ANGLE"]),
                    ),
                )
                ...
                east = Line(
                    ...
                    p1=Vector3(x=TE_mean_line(1).x, y=TE_mean_line(1).y, z=TE_bot(1).z),
                )
                ...
                raise Exception(
                    "Tail option = {} not supported.".format(
                        wing_geometry["TAIL_OPTION"]
                    )
                )
            ...
            if "CLOSE_WING" in wing_geometry and wing_geometry["CLOSE_WING"]:
                ...
                TT_top = TT + wing_geometry["FUNC_TOP_THICKNESS"](
                    x=TT.x, y=TT.y, z=TT.z
                )
                TT_bot = TT + wing_geometry["FUNC_BOT_THICKNESS"](
                    x=TT.x, y=TT.y, z=TT.z
                )
                ...
                interior_flap_patch = CoonsPatch(
                    north=Line(p0=TE_top(0), p1=TE_point),
