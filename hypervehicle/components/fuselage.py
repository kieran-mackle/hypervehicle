    ...
    if len(fuselage_geometries) > 0:
        ...
        for fuse_no, fuselage_geometry in enumerate(fuselage_geometries):
            ...
            if fuselage_geometry is not None:
                offset = (
                    fuselage_geometry["OFFSET"]
                    if "OFFSET" in fuselage_geometry
                    else None
                )
                ...
                if revolve_line is not None:
                    ...
                    if verbosity > 1:
                        print(
                            "  Fuselage nose - {}".format(
                                fuselage_geometry["FUSELAGE_NOSE_OPTION"]
                            )
                        )
                    if fuselage_geometry["FUSELAGE_NOSE_OPTION"] == "sharp-cone":
                        # create cylinder-0
                        patch_dict["cone_0_n"] = ConePatch(
                            x0=Xn,
                            x1=X1,
                            r0=0.0,
                            r1=R1,
                        ...
                        patch_dict["cone_0_e"] = RotatedPatch(
                            underlying_surf=patch_dict["cone_0_n"],
                            angle=np.deg2rad(90.0),
                        )
                        patch_dict["cone_0_s"] = RotatedPatch(
                            underlying_surf=patch_dict["cone_0_n"],
                            angle=np.deg2rad(180.0),
                        )
                        patch_dict["cone_0_w"] = RotatedPatch(
                            underlying_surf=patch_dict["cone_0_n"],
                    ...
                    patch_dict["cone_1_e"] = RotatedPatch(
                        underlying_surf=patch_dict["cone_1_n"], angle=np.deg2rad(90.0)
                    )
                    patch_dict["cone_1_s"] = RotatedPatch(
                        underlying_surf=patch_dict["cone_1_n"], angle=np.deg2rad(180.0)
                    )
                    patch_dict["cone_1_w"] = RotatedPatch(
                        underlying_surf=patch_dict["cone_1_n"], angle=np.deg2rad(270.0)
                    )

                    # create cylinder-2
                    patch_dict["cone_2_n"] = ConePatch(
                        x0=X2,
                        x1=X3,
                        r0=R2,
                        r1=R3,
                    ...
                    patch_dict["cone_2_e"] = RotatedPatch(
                        underlying_surf=patch_dict["cone_2_n"], angle=np.deg2rad(90.0)
                    )
                    patch_dict["cone_2_s"] = RotatedPatch(
                        underlying_surf=patch_dict["cone_2_n"], angle=np.deg2rad(180.0)
                    )
                    patch_dict["cone_2_w"] = RotatedPatch(
                        underlying_surf=patch_dict["cone_2_n"], angle=np.deg2rad(270.0)
                    )

                    if verbosity > 1:
                        print(
                            "  Fuselage tail - {}".format(
                                fuselage_geometry["FUSELAGE_TAIL_OPTION"]
                            )
                        )
                    ...
                    if fuselage_geometry["FUSELAGE_TAIL_OPTION"].lower() == "flat":
                        ...
                        patch_dict["tail_edge_patch_e"] = RotatedPatch(
                            underlying_surf=tail_edge_patch_n, angle=np.deg2rad(90.0)
                        )
                        patch_dict["tail_edge_patch_s"] = RotatedPatch(
                            underlying_surf=tail_edge_patch_n, angle=np.deg2rad(180.0)
                        )
                        patch_dict["tail_edge_patch_w"] = RotatedPatch(
                            underlying_surf=tail_edge_patch_n, angle=np.deg2rad(270.0)
                        )

                    elif (
                        fuselage_geometry["FUSELAGE_TAIL_OPTION"].lower()
                        == "sharp-cone"
                    ):
                        # create cylinder-0
                        patch_dict["cone_3_n"] = ConePatch(
                            x0=X3,
                            x1=X4,
                            r0=R2,
                            r1=0.0,
                        ...
                        patch_dict["cone_3_e"] = RotatedPatch(
                            underlying_surf=patch_dict["cone_3_n"],
                            angle=np.deg2rad(90.0),
                        )
                        patch_dict["cone_3_s"] = RotatedPatch(
                            underlying_surf=patch_dict["cone_3_n"],
                            angle=np.deg2rad(180.0),
                        )
                        patch_dict["cone_3_w"] = RotatedPatch(
                            underlying_surf=patch_dict["cone_3_n"],
                            angle=np.deg2rad(270.0),
                        )
