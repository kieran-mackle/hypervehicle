...
class ParametricMdof(Generator):
    def __init__(self, **kwargs) -> None:
        ...
        self.generate_wings = False #True
        ...
        self.radius = 0.005                         # m, Large body radius
        ...
        self.wing_length = 0.0835                      # m, rear wing length
        self.wing_span = 0.035                      # m, rear wing span
        self.wing_thickness = 0.006                 # m, wing max thickness
        self.wing_le = 0.0002                       # m, wing leading edge radius
        self.wing_maxT = 0.52                        # point of maximum wing thickness, percent of chord
        ...
        self.canard_length = 0.024                  # m, canard length
        self.canard_angle = 0.0                     # deg, canard angle - pivots about point defined below
    ...
    def create_instance(self):
        ...
        # bezier points
        # bez1 = Vector3(x=0., y=0.0, z=-self.height)
        ...
        p3 = p0 + Vector3(x=1.6667*self.canard_length, y=0) + shift_in
        ...
        def canard_thickness_function(thickness: float, side: str):
            ...
            def tf(x: float, y: float, z: float = 0):
                ...
                y_factor_r = (y - p0.y) / self.canard_span
                y_factor_f = (y - p3.y) / self.canard_span
                if x_rel <= 0:
                    canard_slope = can_slope_rear_in*(1-y_factor_r) + can_slope_rear_out*y_factor_r
                    z_local = m * (thickness/2 - canard_slope * abs(x_rel))

                else:
                    canard_slope = can_slope_front_in*(1-y_factor_f) + can_slope_front_out*y_factor_f
                    z_local = m * (thickness/2 - canard_slope * abs(x_rel))
        ...
        p2 = p1 + Vector3(x=self.wing_length/2, y=0) 
        p3 = p0 + Vector3(x=self.wing_length, y=0)
        wing_max_thickness = Vector3(x=self.wing_maxT * (p1.x + p2.x), y=p0.y)
        print('wing ','p0:',p0, 'p1:', p1, 'p2:',p2, 'p3:',p3)        
        ...
        print(f"wing_slope_rear_in={wing_slope_rear_in}, wing_slope_rear_out={wing_slope_rear_out}, wing_slope_front_in={wing_slope_front_in}, wing_slope_front_out={wing_slope_front_out}")
        ...
        def wing_thickness_function(thickness: float, side: str):
            ...
            def tf(x: float, y: float, z: float = 0):
                ...
                # print(x_rel)
                y_factor_r = (y - p0.y) / self.wing_span
                y_factor_f = (y - p3.y) / self.wing_span
                if x_rel <= 0:
                    wing_slope = wing_slope_rear_in*(1-y_factor_r) + wing_slope_rear_out * y_factor_r
                    z_local = m * (thickness/2 - wing_slope * abs(x_rel))
                else:
                    wing_slope = wing_slope_front_in*(1-y_factor_f) + wing_slope_front_out * y_factor_f
                    z_local = m * (thickness/2 - wing_slope * abs(x_rel))
