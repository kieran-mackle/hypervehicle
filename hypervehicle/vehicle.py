...
class Vehicle:
    ...
    def _read_inputs(global_config: dict) -> dict:
        ...
        for option, value in global_config.items():
            if option not in default_config:
                raise Exception(
                    f'Invalid configuration variable "{option}"'
    ...
    def _check_inputs(global_config: dict) -> None:
        ...
        for component in ["WING", "FIN"]:
            if (f"{component}_GEOMETRY_DICT" in global_config) and (
                type(global_config[f"{component}_GEOMETRY_DICT"]) != list
            ):
                global_config[f"{component}_GEOMETRY_DICT"] = [
                    global_config[f"{component}_GEOMETRY_DICT"]
                ]
            ...
            if option == "WING_GEOMETRY_DICT":
                ...
                for wing_geom_dict in global_config["WING_GEOMETRY_DICT"]:
                    for wing_option, valid_wing_inputs in valid_inputs.items():
                        if wing_geom_dict[wing_option] not in valid_wing_inputs:
                            raise Vehicle.InvalidInputError(
                                wing_option, valid_wing_inputs
                            )
                ...
                if global_config["FUSELAGE_GEOMETRY_DICT"] is not None:
                    for fuse_option, valid_fuse_inputs in valid_inputs.items():
                        if (
                            global_config["FUSELAGE_GEOMETRY_DICT"][fuse_option]
                            not in valid_fuse_inputs
                        ):
                            raise Vehicle.InvalidInputError(
                                fuse_option, valid_fuse_inputs
                            )
    ...
    class InvalidInputError(Exception):
        ...
        def __init__(self, option, valid_inputs):
            self.option = option
            self.message = (
                f'Value provided for option "{option}" is '
    ...
    def create_template(directory: str = None) -> None:
        ...
        if directory is not None:
            ...
            template_job_location = os.path.join(
                install_dir, "bin", "hypervehicle_template.py"
            )
            shutil.copy(template_job_location, directory)
        else:
            raise Exception(
                "Please provide the absolute path to your " + "working directory."
            )
        ...
        for op in reqd_options:
            if op not in uo_dict:
                raise Exception(
                    "".join(
                        (
                            "bulk_run_CFD.py requires argument '",
                            op,
                            "', but this argument was not provided.\n",
                        )
                    )
                )
