...
patches can be generated using the command below.
...
### STL Generation
To write the component patches to STL files, use the command below.
...
`Vehicle._enumerated_components`. Then, use the `analyse` command, as
shown below. This will also save the vehicle's inertial properties as
attributes.
